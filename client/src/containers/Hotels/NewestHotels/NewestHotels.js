...
class AllHotels extends Component {
   ...
   componentDidMount() {
      this.props.getNewestHotels();

      // this.props.holdHotelId(35);
   }

   componentWillReceiveProps(nextProps) {
      if(nextProps.hotel.newestHotels) {
         const newAllHotels = nextProps.hotel.newestHotels;

         this.setState({newestHotels: newAllHotels});
      }
   }


   render() {
      if(this.props.hotel.newestHotels !== null ) { //or check the loading state to be 11
         const allHotelView = this.props.hotel.newestHotels.map(hotel => {
...
AllHotels.propTypes = {
   ...
   getNewestHotels: PropTypes.func.isRequired,
...
export default connect(mapStateToProps, {getAllHotels, holdHotelId, releaseHotelId, getTopRatedHotels, getNewestHotels})(withRouter(AllHotels));
